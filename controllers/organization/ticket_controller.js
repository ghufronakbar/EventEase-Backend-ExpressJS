...
exports.ticketShow = async (req, res) => {
    ...
    const qTicketShow = `
        SELECT 
            ...
            e.payment_information, e.event_start, e.event_end, e.created_at 
        FROM tickets AS t 
...
exports.ticketAdd = async (req, res) => {
    const { type, amount, sold, price, date_start, date_end, id_event } = req.body;
    const qTicketAdd = `INSERT INTO tickets(id_event,type,amount,sold,price,date_start,date_end) VALUES(?,?,?,?,?,?,?)`;
    const vTicketAdd = [id_event, type, amount, sold, price, date_start, date_end];

    if (!type || !amount || !sold || !price || !date_start || !date_end) {
        ...
        if (startDate < now) {
            return res.status(401).json({ status: 401, message: "Start time must not be earlier than the current time." });
            ...
            connection.query(qTicketAdd, vTicketAdd, (error, rows, result) => {
                if (error) {
                    ...
                    return res.status(200).json({ status: 200, message: "Ticket added successfully" });
                }
            });
        }
    }
        ...
        if (startDate < now) {
            ...
            connection.query(qTicketEdit, vTicketEdit, (error, rows, result) => {
                if (error) {
                    ...
                    return res.status(200).json({ status: 200, message: "Ticket edited successfully" });
                }
            });
        }
    }
...
exports.ticketSetAmount = async (req, res) => {
    ...
    connection.query(qValidateAmount, id_ticket,
        async (error, rows, result) => {
            if (error) {
                console.log(error);
                return res.status(500).json({ status: 500, message: "Internal Server Error" });
            } else {
                ...
                if (amount < vSold) {
                    ...
                    await connection.query(qTicketAmount, vTicketAmount,
                        (error, rows, result) => {
                            if (error) {
                                console.log(error);
                                return res.status(500).json({ status: 500, message: "Internal Server Error" });
                            } else {
                                return res.status(200).json({ status: 200, message: "Set ticket amount successfully" });
                            }
                        }
                    )
                }
...
exports.ticketDelete = async (req, res) => {
    ...
    connection.query(vTicketDelete, id_ticket,
        async (error, rows, result) => {
            if (error) {
                console.log(error);
                return res.status(500).json({ status: 500, message: "Internal Server Error" });
